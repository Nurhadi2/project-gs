    ...
    <div class="card-body">
        <div class="row">
            ...
            <div class="col-12 mb-3">
                <form action="<?= base_url('petani/update/' . $petani['id_petani']) ?>" method="post">
                    <?= csrf_field() ?>
                    ...
                    <label class="form-label" for="id_kelompok_tani">Kelompok Tani:</label>
                    <select class="form-control" type="text" id="id_kelompok_tani" name="id_kelompok_tani" required>
                        <option value="">Select Kelompok Tani</option>
                        <?php foreach ($kemlompok_tani as $row): ?>
                            <?php if ($row["id_kelompok_tani"] == $petani["id_kelompok_tani"]): ?>
                                <option value="<?= $row["id_kelompok_tani"] ?>" selected><?= $row["nama_kelompok_tani"] ?></option>
                            <?php else: ?>
                                <option value="<?= $row["id_kelompok_tani"] ?>"><?= $row["nama_kelompok_tani"] ?></option>
                            <?php endif ?>
                        <?php endforeach ?>
                    </select>
